...
// "/login" redirects an already-signed-in visitor on to the
// dashboard (see below); the signup/reset flow paths don't — a
// couple of their steps (the confirm-signup redirect, the "set new
// password" step of a reset) run with a real, active Supabase
// session, and that session must not be bounced away.
...
export async function updateSession(request) {
  ...
  if (matches(AUTH_FLOW_PATHS, pathname)) return response;
