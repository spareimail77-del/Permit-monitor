...
export default async function PermitDetailPage({ params }) {
  ...
  const targetRef = decodeURIComponent(params.reference);
  const raw = data.permits.find((p) => p.reference === targetRef);
        ...
        <section style={styles.body}>
          ...
          <div style={styles.notFound}>
            ...
            <p style={{ color: "var(--color-ink-muted)" }}>
              No permit with reference <span className="mono">{targetRef}</span>{" "}
              in the current data. It may have been removed, or the
